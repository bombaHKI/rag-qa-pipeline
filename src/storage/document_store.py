...
from src.config import (
    DATA_DIR, CHUNK_SIZE, CHUNK_OVERLAP, DEFAULT_URLS,
    WIKI_LANGUAGE, WIKI_USER_AGENT, SKIP_SECTIONS,
)
...
class DocumentStore:
    ...
    def _extract_section_text(section: wikipediaapi.WikipediaPageSection) -> str:
        """Recursively extract text from a section and its subsections, skipping non-prose sections."""
    ...
    def _next_id(self) -> str:
        ...
        existing = [int(d["id"]) for d in docs if d["id"].isdigit()]
    ...
    def add_document(self, url: str) -> tuple[bool, str, Optional[str]]:
        """Fetch a Wikipedia article, chunk it, embed it, and store in ChromaDB.
        Returns (success, message, doc_id)."""
        is_valid, title = self._validate_wikipedia_url(url)
        if not is_valid:
            return False, "Invalid Wikipedia URL. Expected: https://en.wikipedia.org/wiki/ArticleTitle", None
        ...
        self.embedding_store.add_chunks(doc_id=doc_id, chunks=chunks, title=title, url=url)
        logger.info(f"Document added: {title} (ID: {doc_id}, {len(chunks)} chunks)")
    ...
    def add_documents(self, urls: list[str]) -> tuple[int, int, list[dict]]:
        ...
        for url in urls:
            ...
            results.append({"url": url, "success": ok, "message": message, "title": title or "Unknown"})
            if ok:
                success_count += 1
            else:
                failure_count += 1
    ...
    def get_random_articles(self, count: int = 5) -> list[str]:
        """Fetch random valid Wikipedia article URLs, excluding already-ingested ones.
        Uses the MediaWiki API random endpoint."""
